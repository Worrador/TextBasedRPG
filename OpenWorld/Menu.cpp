...
Player Menu::playerCreationMenu()
{
...

    {
        ...
        if (selectedMenuPoint == 0) {
            ...
            _getch();
            std::cout << " \x10 " << "If you want to step out of the current menu, use the ESC key." << std::endl << std::endl;
            if (ESCAPE == _getch())
            {
                std::cout << "   " << "No, this one can't be escaped. But nice try." << std::endl;
                std::this_thread::sleep_for(std::chrono::milliseconds(1000));
            }
            std::cout << " \x10 " << "In the game you can: travel, rest, look at your map (which is empty at first) " << std::endl << "   or use / equip / unequip or buy / sell items in the appropriate shops." << std::endl;
            if (ESCAPE == _getch())
            {
